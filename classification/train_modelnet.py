...
from data_processing.hierarchical_modelnet import (
    ModelNetHierarchicalDataset,
    ModelNetHierarchicalDatasetEval,
...
def parse_option():
    parser = argparse.ArgumentParser(description='Training/finetuning on ModelNet40 Rendered Images Dataset')
    ...
    parser.add_argument('--root-dir', default='', type=str,
                        help='ModelNet40 rendered images root directory')
    parser.add_argument('--split', default='', type=str,
                        help='train/test split subdirectory name (if different from "train"/"test")')
    ...
    parser.add_argument('--num-classes', type=int, default=40,
                        help='number of ModelNet40 categories')
...
def main():
    ...
    dataloaders_dict, sampler = load_modelnet_hierarchical(root_dir, args)
...
def load_modelnet_hierarchical(root_dir, opt):
    ...
    train_split = opt.split if opt.split else 'train'
    test_split = 'test'

    train_dataset = ModelNetHierarchicalDataset(
        root_dir=root_dir,
        split=train_split,
        transform=TwoCropTransform(train_transform),
    )

    # Memory bank uses train set with eval transform
    memory_dataset = ModelNetHierarchicalDatasetEval(
        root_dir=root_dir,
        split=train_split,
        transform=test_transform,
    )

    test_dataset = ModelNetHierarchicalDatasetEval(
        root_dir=root_dir,
        split=test_split,
        transform=test_transform,
    )
...
def test(net, memory_data_loader, test_data_loader, args, epoch, device):
    ...
    with torch.no_grad():
        ...
        for data, labels in tqdm(memory_data_loader, desc='Feature extracting'):
            ...
            label_bank.append(labels[0])  # use category (coarsest, 40 classes) label
        # [D, N]
