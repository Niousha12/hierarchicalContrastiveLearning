...
from data_processing.hierarchical_inat import (
    INatHierarchicalDataset,
    INatHierarchicalDatasetEval,
...
def parse_option():
    parser = argparse.ArgumentParser(description='Training/finetuning on iNaturalist Dataset')
    ...
    parser.add_argument('--root-dir', default='', type=str,
                        help='iNaturalist image root directory')
    parser.add_argument('--ann-file-train', default='', type=str,
                        help='path to iNat train JSON annotation file')
    parser.add_argument('--ann-file-val', default='', type=str,
                        help='path to iNat val JSON annotation file')
    parser.add_argument('--hierarchy-file', default='', type=str,
                        help='optional JSON mapping category_id to genus string')
    parser.add_argument('--query-ratio', type=float, default=0.2,
                        help='fraction of val set used as query (default: 0.2)')
    ...
    parser.add_argument('--crop-size', default=224, type=int,
                        help='crop size')
    parser.add_argument('--num-classes', type=int, default=8142,
    ...
    parser.add_argument('--pretrained', dest='pretrained', action='store_false',
                        help='disable pre-trained model (default: use ImageNet pretrained weights)')
    ...
    parser.add_argument('--eval-freq', type=int, default=1,
                        help='evaluate every N epochs; epochs 1-3 are always evaluated (default: 1)')
...
def main():
    ...
    dataloaders_dict, sampler = load_inat_hierarchical(
        root_dir, args.ann_file_train, args.ann_file_val, args)
...
def load_inat_hierarchical(root_dir, ann_file_train, ann_file_val, opt):
    ...
    hierarchy_file = opt.hierarchy_file if opt.hierarchy_file else None

    train_dataset = INatHierarchicalDataset(
        root_dir=root_dir,
    ...
    # Memory bank: train set with eval (single-crop) transform
    memory_dataset = INatHierarchicalDatasetEval(
        root_dir=root_dir,
        ann_file=ann_file_train,
    ...
    # Query/gallery split from val set: sort by index, take first query_ratio as query
    val_dataset_full = INatHierarchicalDatasetEval(
        root_dir=root_dir,
        ann_file=ann_file_val,
        transform=test_transform,
        hierarchy_file=hierarchy_file,
    )
    n_val = len(val_dataset_full)
    n_query = max(1, int(n_val * opt.query_ratio))
    # Deterministic split: first n_query as query, rest as gallery
    query_indices = list(range(n_query))
    gallery_indices = list(range(n_query, n_val))

    query_dataset = torch.utils.data.Subset(val_dataset_full, query_indices)
    gallery_dataset = torch.utils.data.Subset(val_dataset_full, gallery_indices)

    print('LENGTH TRAIN', len(train_dataset))
    print('LENGTH VAL (query)', len(query_dataset), '(gallery)', len(gallery_dataset))
    ...
    # Use gallery as memory for KNN and query as test
    dataloaders_dict['test'] = torch.utils.data.DataLoader(
        query_dataset, batch_size=opt.batch_size,
        shuffle=False, num_workers=16,
    )
...
def test(net, memory_data_loader, test_data_loader, args, epoch, device):
    ...
    with torch.no_grad():
        ...
        for data, labels in tqdm(memory_data_loader, desc='Feature extracting'):
            ...
            label_bank.append(labels[1])  # use species (finest) label
        # [D, N]
